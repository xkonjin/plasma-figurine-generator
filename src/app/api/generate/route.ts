import { NextRequest, NextResponse } from "next/server";
import { trackServerEvent } from "@/lib/posthog-server";

const GEMINI_API_KEY = process.env.GEMINI_API_KEY;
const GEMINI_API_URL =
  "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image:generateContent";

// Plasma brand colors
const PLASMA_GREEN = "#162F29";

// Helper to fetch logo and convert to base64 at runtime
async function fetchLogoAsBase64(): Promise<string> {
  try {
    // Use absolute URL for the logo
    const logoUrl = "https://plasma-figurine-generator.vercel.app/plasma-logo.png";
    const response = await fetch(logoUrl);
    if (!response.ok) {
      console.error("Failed to fetch logo:", response.status);
      return "";
    }
    const buffer = await response.arrayBuffer();
    const base64 = Buffer.from(buffer).toString("base64");
    console.log(`Fetched logo from URL, base64 length: ${base64.length}`);
    return base64;
  } catch (error) {
    console.error("Error fetching logo:", error);
    return "";
  }
}

// Plasma logo as base64 PNG (166x166, white spiral on dark green)
const PLASMA_LOGO_BASE64 =
  "";

// Creative logo placement variations - randomly selected for each generation
const LOGO_PLACEMENTS = [
  // Jewelry & Accessories - Gold
  {
    name: "gold_necklace",
    instruction: `LOGO PLACEMENT: A beautiful small GOLD PENDANT NECKLACE featuring the Plasma spiral logo from the reference image. The pendant is delicate (about 1cm), made of polished 18k gold with the exact spiral design. It hangs on a fine gold chain, catching studio light elegantly. This is the ONLY place the logo appears.`,
  },
  {
    name: "gold_lapel_pin",
    instruction: `LOGO PLACEMENT: A sophisticated GOLD LAPEL PIN on their jacket/sweater chest featuring the exact Plasma spiral from the reference image. Small (1cm), polished gold, positioned like a luxury brand pin. This is the ONLY place the logo appears.`,
  },
  {
    name: "gold_signet_ring",
    instruction: `LOGO PLACEMENT: An elegant GOLD SIGNET RING on their finger with the Plasma spiral logo engraved/embossed into the face. The spiral from the reference image is precisely rendered in polished gold. This is the ONLY place the logo appears.`,
  },
  {
    name: "gold_bracelet_charm",
    instruction: `LOGO PLACEMENT: A delicate GOLD CHARM BRACELET with a single charm featuring the Plasma spiral logo from the reference image. The charm is small, polished gold, catching the light beautifully. This is the ONLY place the logo appears.`,
  },
  {
    name: "gold_tie_clip",
    instruction: `LOGO PLACEMENT: A refined GOLD TIE CLIP or COLLAR CLIP with the Plasma spiral logo from the reference image engraved into it. Elegant, professional, polished gold finish. This is the ONLY place the logo appears.`,
  },
  {
    name: "gold_earring",
    instruction: `LOGO PLACEMENT: A single small GOLD STUD EARRING featuring the Plasma spiral logo from the reference image. Minimalist, elegant, polished gold - visible but subtle. This is the ONLY place the logo appears.`,
  },

  // Environment & Base
  {
    name: "wood_inlay_base",
    instruction: `LOGO PLACEMENT: The circular BASE/PEDESTAL of the figurine features the Plasma spiral logo from the reference image as a beautiful WOOD MARQUETRY inlay - darker walnut spiral pattern inlaid into lighter maple wood. Museum-quality craftsmanship. This is the ONLY place the logo appears.`,
  },
  {
    name: "brass_engraved_base",
    instruction: `LOGO PLACEMENT: The figurine stands on an elegant circular BASE with the Plasma spiral logo from the reference image engraved into brushed BRASS. The spiral catches light beautifully, like a luxury collectible stand. This is the ONLY place the logo appears.`,
  },
  {
    name: "marble_etched_base",
    instruction: `LOGO PLACEMENT: The figurine's BASE is white marble with the Plasma spiral logo from the reference image subtly etched and filled with gold leaf. Elegant, museum-quality presentation. This is the ONLY place the logo appears.`,
  },
  {
    name: "terrazzo_floor",
    instruction: `LOGO PLACEMENT: The FLOOR/GROUND beneath the figurine is stylized terrazzo with the Plasma spiral logo from the reference image formed by arranged stone chips in dark green against cream. Artistic and architectural. This is the ONLY place the logo appears.`,
  },

  // Tech & Objects
  {
    name: "laptop_decal",
    instruction: `LOGO PLACEMENT: Their LAPTOP has a small brushed gold/brass metal decal of the Plasma spiral logo from the reference image on the lid. Not a sticker - a premium metal emblem like luxury brands use. This is the ONLY place the logo appears.`,
  },
  {
    name: "mug_emblem",
    instruction: `LOGO PLACEMENT: A ceramic MUG nearby (or being held) in dark forest green with the Plasma spiral logo from the reference image in gold/brass as an elegant printed or embossed design. This is the ONLY place the logo appears.`,
  },
  {
    name: "notebook_foil",
    instruction: `LOGO PLACEMENT: A leather NOTEBOOK/JOURNAL nearby in dark green with the Plasma spiral logo from the reference image embossed in gold foil on the cover. Premium stationery aesthetic. This is the ONLY place the logo appears.`,
  },
  {
    name: "phone_case",
    instruction: `LOGO PLACEMENT: Their PHONE has a dark green case with a small gold metallic Plasma spiral logo from the reference image - like a luxury brand phone case. Subtle but visible. This is the ONLY place the logo appears.`,
  },
  {
    name: "water_bottle",
    instruction: `LOGO PLACEMENT: A premium WATER BOTTLE nearby in matte dark green with the Plasma spiral logo from the reference image laser-etched or printed in gold. Modern, minimal design. This is the ONLY place the logo appears.`,
  },

  // Clothing Details
  {
    name: "embroidered_chest",
    instruction: `LOGO PLACEMENT: Small GOLD THREAD EMBROIDERY of the Plasma spiral logo from the reference image on the chest of their dark green sweater/jacket. About 2cm, like a subtle luxury brand logo. Realistic embroidery texture. This is the ONLY place the logo appears.`,
  },
  {
    name: "sleeve_patch",
    instruction: `LOGO PLACEMENT: A small embroidered PATCH on their upper sleeve featuring the Plasma spiral logo from the reference image in gold thread on dark green fabric. Like a quality brand detail. This is the ONLY place the logo appears.`,
  },
  {
    name: "pocket_embroidery",
    instruction: `LOGO PLACEMENT: Tiny gold thread embroidery of the Plasma spiral logo from the reference image on their shirt/jacket POCKET. Subtle, refined, like bespoke tailoring. This is the ONLY place the logo appears.`,
  },
];

function getRandomLogoPlacement(): { name: string; instruction: string } {
  const index = Math.floor(Math.random() * LOGO_PLACEMENTS.length);
  return LOGO_PLACEMENTS[index];
}

function buildPrompt(
  activity: string,
  customPrompt: string
): { prompt: string; placementName: string } {
  const placement = getRandomLogoPlacement();

  const basePrompt = `Create a miniature, full-body, isometric, hyper-realistic 3D collectible figurine of this person.

POSE & ACTIVITY: ${activity}

FIGURINE STYLE:
- Isometric view (45-degree elevated angle looking down)
- High-end collectible figurine aesthetic (like premium designer toys)
- Hyper-realistic rendering with soft, diffused studio lighting
- Clean white or very light gray background
- Subtle shadow underneath for grounding
- Museum-quality detail and finish

OUTFIT:
- Dark forest green sweater, hoodie, or casual jacket (color: ${PLASMA_GREEN})
- Well-fitted dark navy or charcoal pants
- Clean, modern, professional-casual style
- Quality fabric textures

${placement.instruction}

PLASMA LOGO DESCRIPTION (for the branding element):
The Plasma logo is a precise geometric spiral shape. It looks like a stylized hurricane or whirlpool viewed from above - a clean logarithmic spiral that curves elegantly from outer edge to inner center point. The spiral has approximately 3-4 smooth rotations, tapering gradually as it approaches the center. Think of a perfectly geometric Fibonacci spiral or a nautilus shell cross-section. The spiral should be rendered in WHITE or gold/brass/silver depending on the material context. Do NOT make a generic swirl with random curves - this is a precise, mathematical spiral shape.

MOOD: Warm, approachable, confident - someone who genuinely enjoys their work.

QUALITY: Ultra-high detail, photorealistic materials, perfect lighting, 4K resolution.`;

  const finalPrompt = customPrompt
    ? `${basePrompt}\n\nADDITIONAL USER DETAILS: ${customPrompt}`
    : basePrompt;

  return { prompt: finalPrompt, placementName: placement.name };
}

export async function POST(request: NextRequest) {
  try {
    if (!GEMINI_API_KEY) {
      return NextResponse.json(
        { error: "API key not configured" },
        { status: 500 }
      );
    }

    const body = await request.json();
    const { image, name, activity, customPrompt } = body;

    if (!image) {
      return NextResponse.json({ error: "No image provided" }, { status: 400 });
    }

    // Extract base64 data from data URL
    const base64Match = image.match(/^data:image\/(\w+);base64,(.+)$/);
    if (!base64Match) {
      return NextResponse.json(
        { error: "Invalid image format" },
        { status: 400 }
      );
    }

    const mimeType = `image/${base64Match[1]}`;
    const imageData = base64Match[2];

    const { prompt, placementName } = buildPrompt(
      activity || "working at a laptop",
      customPrompt || ""
    );

    // Build simple single-image request
    const requestBody = {
      contents: [
        {
          parts: [
            {
              inlineData: {
                mimeType,
                data: imageData,
              },
            },
            {
              text: `REFERENCE IMAGE: Photo of ${name || "the person"} - use their exact face, hair, and skin tone for the figurine.

${prompt}`,
            },
          ],
        },
      ],
      generationConfig: {
        responseModalities: ["TEXT", "IMAGE"],
      },
    };

    // Log request size for debugging
    console.log(`Generating figurine with logo placement: ${placementName}`);
    console.log(`User image base64 size: ${imageData.length} chars`);

    const response = await fetch(GEMINI_API_URL, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-goog-api-key": GEMINI_API_KEY || "",
      },
      body: JSON.stringify(requestBody),
    });

    if (!response.ok) {
      const errorText = await response.text();
      console.error("Gemini API error:", response.status, errorText);

      let errorMessage = `Generation failed: ${response.status}`;
      try {
        const errorJson = JSON.parse(errorText);
        if (errorJson.error?.message) {
          errorMessage = errorJson.error.message;
        }
      } catch {
        // Use default message
      }

      return NextResponse.json(
        { error: errorMessage, details: errorText.substring(0, 500) },
        { status: response.status }
      );
    }

    const data = await response.json();

    // Extract generated images
    const images: string[] = [];
    const candidates = data.candidates || [];

    for (const candidate of candidates) {
      const parts = candidate.content?.parts || [];
      for (const part of parts) {
        if (part.inlineData?.data) {
          const imgMimeType = part.inlineData.mimeType || "image/png";
          images.push(`data:${imgMimeType};base64,${part.inlineData.data}`);
        }
      }
    }

    if (images.length === 0) {
      if (data.promptFeedback?.blockReason) {
        await trackServerEvent(name || "anonymous", "figurine_generation_blocked", {
          reason: data.promptFeedback.blockReason,
          activity,
        });
        return NextResponse.json(
          { error: `Content blocked: ${data.promptFeedback.blockReason}` },
          { status: 400 }
        );
      }
      await trackServerEvent(name || "anonymous", "figurine_generation_failed", {
        reason: "no_image_generated",
        activity,
      });
      return NextResponse.json(
        { error: "No image generated. Try a different prompt or photo." },
        { status: 500 }
      );
    }

    // Track successful generation
    await trackServerEvent(name || "anonymous", "figurine_generated", {
      activity,
      logoPlacement: placementName,
      imagesCount: images.length,
    });

    return NextResponse.json({ 
      images,
      logoPlacement: placementName 
    });
  } catch (error) {
    console.error("Generation error:", error);
    await trackServerEvent("system", "figurine_generation_error", {
      error: error instanceof Error ? error.message : "Unknown error",
    });
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unknown error" },
      { status: 500 }
    );
  }
}
